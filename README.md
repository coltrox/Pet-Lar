...
Usuario_Formulario (Relacionamento N:N)

id_usuario (FK → Usuario.id_usuario)

id_formulario (FK → Formulario.id_formulario)

PK composta: (id_usuario, id_formulario)
...
nome, especie, raca, data_nascimento, sexo, origem

id_ong (FK → ONG.id_ong)

id_carteira (FK → Carteira_Vacinacao.id_carteira, UNIQUE)

Carteira_Vacinacao

id_carteira (PK)

id_pet (FK → Pet.id_pet)

Vacina

id_vacina (PK)

id_carteira (FK → Carteira_Vacinacao.id_carteira)

nome_vacina, data_aplicacao
...
data_solicitacao, status, observacao

Adocao

id_adocao (PK)

id_solicitacao (FK → Solicitacao_Adocao.id_solicitacao, UNIQUE)
...
Usuario_Formulario → Usuario & Formulario: N:N

ONG → Endereco: 1:N

Pet → ONG: N:1

Pet → Carteira_Vacinacao: 1:1

Carteira_Vacinacao → Vacina: 1:N

Solicitacao_Adocao → Usuario, Pet, ONG: N:1 para cada

Adocao → Solicitacao_Adocao: 1:1
